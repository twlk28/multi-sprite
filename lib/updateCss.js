...
module.exports = function (css, imageReplaces) {
    ...
    if (resources !== null) {
        for (var i = 0; i < resources.length; i++) {
            ...
            if (isNeedProcessRem(data)) {
                img = imageReplaces[ path.normalize(pathToResource) ]
                if (img) {
                    data = calcRem(data, pathToResource, img, imageReplaces.rootFontSize)
                }
                continue;
            }
            ...
            if (img) {
                ...
                cssSnippet3x += calc3x(selectors, img)
            }
            img = imageReplaces[addSuffix(pathToResource, CONST.SUFFIX_2X)]
            if (img) {
                selectors = selectors || getSelectorsBySlice(data, pathToResource)
                cssSnippet2x += calc2x(selectors, img)
            }
            img = imageReplaces[ path.normalize(pathToResource) ]
            if (img) {
                data = calc1x(data, pathToResource, img)
            }
        }
    }
    ...
    function calcRem(css, url, img, rootFontSize){
        var regExp = new RegExp('background-image:[\\s]?url\\(["\']?'+url+'["\']?\\)', 'ig')
        var pos = 'background-image:url("' + img.sprite +'"); background-size:'+ px2rem(img.spriteWidth, rootFontSize)
                +'; background-position: -'+px2rem(img.x, rootFontSize)+' -'+px2rem(img.y, rootFontSize)
        return css.replace(regExp, pos)
    }
    function calc1x(css, url, img){
        var regExp = new RegExp('background-image:[\\s]?url\\(["\']?'+url+'["\']?\\)', 'ig')
        var pos = 'background-image:url("' + img.sprite +'"); background-position: -'+img.x+'px -'+img.y+'px'
        return css.replace(regExp, pos)
    }
    function calc2x(selectors, img){
    ...
    function calc3x(selectors, img){
        return selectors.map(function(selector){
            var snippet = selector+'{background-image:url("' + img.sprite +'");' +
                'background-position: -'+img.x/3+'px -'+img.y/3+'px; background-size: '+ img.spriteWidth/3 +'px; }'
            return snippet
        }).join('')
    }
    ...
    function px2rem(pxValue, rootFontSize){
